...
const CreateEventForm = forwardRef((props, ref) => {
    ...
    const { data, setData, post, errors, hasErrors, transform } = useForm({
        start: DateTime.fromISO(props.info.dateStr).toFormat(
            "yyyy-MM-dd'T'HH:mm"
        ),
    ...
    const handleSubmit = e => {
        ...
        transform(data => ({
            ...data,
            start: DateTime.fromFormat(data.start, "yyyy-MM-dd'T'HH:mm")
                .toUTC()
            ...
            <div>
                <div className="form-check">
                    ...
                    <label htmlFor="repeat">
                        Repeat this event on this day and time for the next 20
                        weeks
                    </label>
                </div>
                {errors.repeat && (
                    <div className="form-error">{errors.repeat}</div>
                )}
            </div>
        </form>
    )
