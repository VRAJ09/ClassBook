...
const SubjectsForm = forwardRef((props, ref) => {
    ...
    return (
        <form ref={ref} onSubmit={handleSubmit} noValidate>
            <div className="px-6">
                <input
                    className="form-field h-12 mb-3"
                    type="text"
                    value={searchValue}
            ...
            >
                <ul className="space-y-4 pb-6">
                    {allSubjects.map(subject => {
                        if (
                            subject.name.toLowerCase().indexOf(searchValue) > -1
                        ) {
                            return (
                                <SubjectRow
                                    key={subject.id}
                    ...
                    {errors.subjects && (
                        <li className="form-error mt-2 mx-6">
                            {errors.subjects}
                        </li>
                    )}
                </ul>
    ...
    <>
        <li className="mx-6">
            <label
                ...
                htmlFor={props.subject.name}
            >
                <div className="flex justify-between items-center shadow-md py-1 px-3 rounded bg-gray-50">
                    <span className="uppercase sm:text-lg">
                        {props.subject.name}
                    </span>
                    <div className="props-check">
                        <input
                            name={props.subject.id}
                            id={props.subject.name}
                            type="checkbox"
                            checked={props.data[props.subject.id]}
